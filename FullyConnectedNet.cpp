									 ...
									 dtype(dtype)
{
	...
	if (hiddendims.empty()) {
		...
		num_layers = 1;
		Mat first_layer(input_dim, num_classes, CV_32F);
	...
	Mat first_layer(input_dim, hiddendims[0], CV_32F);
	randn(first_layer, 0, weight_scale);
	...
	{//i == num_layers - 3, i + 1 == num_layers - 2
		weights.push_back(Mat(hiddendims[i], hiddendims[i + 1], CV_32F)); //i == num_layers - 3, i + 1 == num_layers - 2
		randn(weights.back(), 0, weight_scale);
	}

	//
	weights.push_back(Mat(hiddendims[num_layers - 2], num_classes, CV_32F));
	randn(weights.back(), 0, weight_scale);
}
...
{ //test mode
	...
	for (int i = 0; i < num_layers; ++i)
	{
		...
		//cout << out[i].rows << '\t' << out[i].cols << endl;
...
pair<double, Mat> softmax_loss(Mat a, Mat b)
{
	...
	*/
	...
	for (int i{0}; i < x.rows; ++i)
	{
		...
		Mat log_probs = x.row(i) - log(z.val[0]);
		double loss = -log_probs.at<float>(0, y.at<int>(i));
		loss_sum += loss;
		Mat probs;
		exp(log_probs, probs);
		Mat dx = probs.clone();
		dx.at<float>(0, y.at<int>(i)) -= 1;
		dx_rows.push_back(dx);
	}
